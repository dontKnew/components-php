...
class user_mail_logs extends MX_Controller {
	public $tb_users;
	public $tb_user_logs;
	...
	public function index(){
		...
		if (!$this->db->table_exists(USER_MAIL_LOGS)) {
			redirect(cn('users'));
		}
		...
		$config = array(
			...
			'total_rows'         => $this->model->get_user_mail_logs_list(true),
		...
		$user_logs = $this->model->get_user_mail_logs_list(false, "all", $limit_per_page, $page * $limit_per_page);
		$data = array(
	...
	public function ajax_search(){
		$k = post("k");
		$user_logs = $this->model->get_user_mail_logs_by_search($k);
		$data = array(
			"module"      => get_class($this),
			"columns"     => $this->columns,
			"user_logs"   => $user_logs,
	...
	public function ajax_delete_item($ids = ""){
		$this->model->delete($this->tb_user_mail_logs, $ids, true);
	}
		...
		switch ($type) {
			case 'delete':
				foreach ($idss as $key => $ids) {
					$this->db->delete($this->tb_user_mail_logs, ['ids' => $ids]);
				}
				ms(array(
					"status"  => "success",
			...
			case 'clear_all':
				$this->db->empty_table($this->tb_user_mail_logs);
				ms(array(
					"status"  => "success",
					"message" => lang("Deleted_successfully")
				));
