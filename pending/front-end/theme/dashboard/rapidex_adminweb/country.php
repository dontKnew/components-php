...
$sql="insert into country(name,code, photo) 
    value('".$name."','".$code."','".$newName."')";
$rs=mysqli_query($con,$sql);
if($rs)
{
$msg="New country has been added successfully.";
$msg=urlencode($msg);
header("Location:country.php?sts=$msg");
}
else
{
...
header("Location:country.php?sts=$msg");
      ...
      echo "<meta http-equiv=\"refresh\" content=\"0;URL=country.php?sts=inserted\">";
}
...
$sql="update country set name='".$name."',code='".$code."',photo='".$newName."'  where id='$id'"; 
include("/home4/elect7wk/rapidexworldwide.com/admin/config/config.php");

$country_date = array("name"=>$name, "code"=>$code);
try {
    $db = getDbInstance();
    $check_test = $db->where("id", $id)->update("country", $country_date);
}catch(Exception $e){
    echo $e->getMessage();
    exit;
}
...
mysqli_query($con,"delete from country where id='$_REQUEST[id]'");
header("location:country.php?msg=Deleted");
}   
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current"  style="text-transform: capitalize;">country</a> </div>
    <h1  style="text-transform: capitalize;">country</h1>
  </div>
  <div class="container-fluid">  <div class="widget-content"> <a href="country.php?action=Add" class="btn btn-success" style="float: right;">Add country</a> 
    ...
    <? } ?>
      <? if($_GET['action']=="edit"){ 
  $s1="select * from country where id ='$_GET[id]'";
    ...
    <? } ?>
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5 style="text-transform: capitalize;">Country list</h5>
          </div>
  ...
  $pages->paginate();
  $i=1;
  $sql=mysqli_query($con,"select * from country order by id desc $pages->limit");
  ...
  ?>
                <tr class="odd gradeX">
                  ...
                  <td> 
<a href="country.php?action=edit&id=<?=$row['id'];?>" class="label   bg_db">Edit</a> 
