...
if(isset($_POST["Submit"]) ){  
  $name=$_POST['name']; 
$sql="insert into rack(name) 
    value('".$name."')";
$rs=mysqli_query($con,$sql);
if($rs)
{
$msg="New rack has been added successfully.";
$msg=urlencode($msg);
header("Location:rack.php?sts=$msg");
}
else
{
...
header("Location:rack.php?sts=$msg");
      ...
      echo "<meta http-equiv=\"refresh\" content=\"0;URL=rack.php?sts=inserted\">";
}
...
$sql="update rack set name='".$name."'  where id='$id'"; 
$rs=mysqli_query($con,$sql);
if($rs)
{
$msg="rack Details has been updated successfully";
$msg=urlencode($msg);
header("Location:rack.php?sts=$msg");
}
else
...
mysqli_query($con,"delete from rack where id='$_REQUEST[id]'");
header("location:rack.php?msg=Deleted");
}   
...
<div id="content">
  <div id="content-header">
    ...
    <h1  style="text-transform: capitalize;">rack</h1>
  </div>
  <div class="container-fluid">  <div class="widget-content"> <a href="rack.php?action=Add" class="btn btn-success" style="float: right;">Add rack</a> 
...
while($rowr=mysqli_fetch_array($sqlr)){
 if (strpos($rowr['name'], 'A') !== false) { 
    $color="030cb0";
}else  if (strpos($rowr['name'], 'B') !== false) { 
    $color="015f19";
}else  if (strpos($rowr['name'], 'C') !== false) { 
    $color="5f0e01";
}else  if (strpos($rowr['name'], 'D') !== false) { 
    $color="5e015f";
}
 ?>
...
<? } ?>
  ...
  $pages->paginate();
  $i=1;
  $sql=mysqli_query($con,"select * from rack order by id desc $pages->limit");
  ...
  ?>
                <tr class="odd gradeX">
                  ...
                  <td> 
<a href="rack.php?action=edit&id=<?=$row['id'];?>" class="label   bg_db">Edit</a> 
