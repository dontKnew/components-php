...
if($rs)
{
$msg="New state has been added successfully.";
$msg=urlencode($msg);
header("Location:state.php?sts=$msg");
}
else
{
...
header("Location:state.php?sts=$msg");
      ...
      echo "<meta http-equiv=\"refresh\" content=\"0;URL=state.php?sts=inserted\">";
}
 ...
 $id=$_POST['id'];  
$name=$_POST['name'];  
$sql="update state set name='$name' where id='$id'"; 
$rs=mysqli_query($con,$sql) or die(mysqli_error($con));
if($rs)
{
$msg="state Details has been updated successfully";
$msg=urlencode($msg);
header("Location:state.php?sts=$msg");
}
else
...
mysqli_query($con,"delete from state where id='$_REQUEST[id]'");

header("location:state.php?msg=Deleted");
}   
...
$del_id = $checkbox[$i];
$sql = "DELETE FROM state WHERE id='$del_id'";
...
for($i=0;$i<count($checkbox);$i++){
$del_id = $checkbox[$i];
$sql = "update state set status='1' WHERE id='$del_id'";
$result = mysqli_query($con,$sql);
}
// if successful redirect to delete_multiple.php
if($result){
...
for($i=0;$i<count($checkbox);$i++){
$del_id = $checkbox[$i];
$sql = "update state set status='0' WHERE id='$del_id'";
$result = mysqli_query($con,$sql);
}
// if successful redirect to delete_multiple.php
if($result){
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current"  style="text-transform: capitalize;">state</a> </div>
    <h1  style="text-transform: capitalize;">state</h1>
  </div>
  <div class="container-fluid">  <div class="widget-content"> <a href="state.php?action=Add" class="btn btn-success" style="float: right;">Add state</a> 
  ...
  $s1="select * from state where id ='$_GET[id]'";
    ...
    <? } ?>
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5 style="text-transform: capitalize;">state list</h5>
          </div>
          <div class="widget-content nopadding">   
  ...
  $pages->paginate();
  $i=1;
  $sql=mysqli_query($con,"select * from state order by id desc $pages->limit");
...
<a href="state.php?action=edit&id=<?=$row['id'];?>" class="label   bg_db">Edit</a> 
