...
mysqli_query($con,"delete from enquiry where id='$_REQUEST[id]'") or die(mysqli_error($con));
...
$del_id = $checkbox[$i];
$sql = "DELETE FROM enquiry WHERE id='$del_id'";
...
for($i=0;$i<count($checkbox);$i++){
$del_id = $checkbox[$i];
$sql = "update enquiry set status='1' WHERE id='$del_id'";
$result = mysqli_query($con,$sql);
}
// if successful redirect to delete_multiple.php
if($result){
...
for($i=0;$i<count($checkbox);$i++){
$del_id = $checkbox[$i];
$sql = "update enquiry set status='0' WHERE id='$del_id'";
$result = mysqli_query($con,$sql);
}
// if successful redirect to delete_multiple.php
if($result){
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="current"  style="text-transform: capitalize;">enquiry</a> </div>
    <h1  style="text-transform: capitalize;">enquiry</h1>
  </div>
  <div class="container-fluid">  <div class="widget-content"> 

     <a href="export-enquiry.php" class="btn btn-success" style="float: right;" target="_blank">Export Enquiry</a>  
  ...
  $s1="select * from enquiry where id ='$_GET[id]'";
    ...
    <? } ?>
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5 style="text-transform: capitalize;">enquiry list</h5>
          </div>
          <div class="widget-content nopadding">  <form name="form1" method="post" action="">
            <table class="table table-bordered table-striped">
              ...
              <tbody>
                <?
    $query = "SELECT COUNT(*) FROM enquiry";
  ...
  $pages->paginate();
  $i=1;
  $sql=mysqli_query($con,"select * from enquiry order by id desc $pages->limit");
...
<a 
href="#" onClick="MyWindow=window.open('enquiry-details.php?action=view&id=<?=$row['id'];?>','MyWindow','width=900,height=500'); return false;" class="label   bg_db">View</a> 
